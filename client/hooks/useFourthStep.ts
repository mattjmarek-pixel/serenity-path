...
export function useFourthStep() {
  ...
  const addResentment = useCallback(
    async (entry: Omit<ResentmentEntry, "id" | "createdAt">) => {
  ...
  const deleteResentment = useCallback(
    async (id: string) => {
      const updated = {
        ...data,
        resentments: data.resentments.filter((e) => e.id !== id),
      };
      return persist(updated);
    },
    [data, persist],
  ...
  const addFear = useCallback(
    async (entry: Omit<FearEntry, "id" | "createdAt">) => {
  ...
  const addHarm = useCallback(
    async (entry: Omit<HarmEntry, "id" | "createdAt">) => {
