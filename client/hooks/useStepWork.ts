...
export function useStepWork() {
  ...
  const saveAnswer = useCallback(async (stepNumber: number, questionIndex: number, answer: string) => {
    try {
      ...
      if (existing) {
        updated = stepWorkData.map(s =>
          s.stepNumber === stepNumber
            ? { ...s, answers: { ...s.answers, [questionIndex]: answer }, lastUpdated: new Date().toISOString() }
            : s
        );
      } else {
  ...
  const getStepData = useCallback((stepNumber: number): StepWorkData | undefined => {
    return stepWorkData.find(s => s.stepNumber === stepNumber);
  }, [stepWorkData]);

  const getStepProgress = useCallback((stepNumber: number, totalQuestions: number): { answered: number; total: number } => {
    const data = stepWorkData.find(s => s.stepNumber === stepNumber);
    if (!data) return { answered: 0, total: totalQuestions };
    const answered = Object.values(data.answers).filter(a => a.trim().length > 0).length;
  ...
  const isStepComplete = useCallback((stepNumber: number, totalQuestions: number): boolean => {
    const data = stepWorkData.find(s => s.stepNumber === stepNumber);
    if (!data) return false;
    const answered = Object.values(data.answers).filter(a => a.trim().length > 0).length;
